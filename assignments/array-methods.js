...
let fullName = [];
let convertData = runners.forEach(runner => (fullName.push(runner.first_name + " " + runner.last_name)));
...
// Problem 1: Identify the highest donors and send them thank you cards

let largestDonors = runners.filter(runner => runner.donation > 250);
console.log(largestDonors);

// Problem 2: Too many runners signed up, so now we need to cut the participants in half for two seperate races

let raceOne = runners.filter(runner => runner.id < 26);
let raceTwo = runners.filter(runner => runner.id > 25);

console.log(raceOne);
console.log(raceTwo);

// Problem 3: Marketing department wants an array that only contains names + email addresses for promotional purposes

let emails = runners.map(runner => runner.first_name + " " + runner.email);
console.log(emails);